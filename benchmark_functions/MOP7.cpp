...
class MOP7 : public NOMAD::Evaluator
{
  public:
    explicit MOP7(const std::shared_ptr<NOMAD::EvalParameters> &evalParams)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB)
    {
    }

    ~MOP7() override = default;
    ...
    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        ...
        try
        {
            ...
            // Objectives
            f1 = (x1 - 2) * (x1 - 2) / 2 + (x2 + 1) * (x2 + 1) / 13 + 3;
            f2 = (x1 + x2 - 3) * (x1 + x2 - 3) / 36 + (-x1 + x2 + 2) * (-x1 + x2 + 2) / 8 - 17;
            f3 = (x1 + 2 * x2 - 1) * (x1 + 2 * x2 - 1) / 175 + (-x1 + 2 * x2) * (-x1 + 2 * x2) / 17 - 13;
...
bool run_pb()
{
    ...
    try
    {
        ...
        NOMAD::ArrayOfDouble lb(n, -400);
        params->setAttributeValue("LOWER_BOUND", lb);

        NOMAD::ArrayOfDouble ub(n, 400);
        ...
        std::string history_name = "MOP7_dmultimads.txt";
        ...
        auto ev = std::make_unique<MOP7>(params->getEvalParams());
