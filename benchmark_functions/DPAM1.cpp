...
class DPAM1 : public NOMAD::Evaluator
{
  public:
    explicit DPAM1(const std::shared_ptr<NOMAD::EvalParameters> &evalParams, const size_t ctype)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB), _n(10), _ctype(ctype)
    {
    }

    ~DPAM1() override = default;
...
bool run_pb(size_t ctype)
{
    ...
    try
    {
        ...
        NOMAD::ArrayOfDouble lb(n, -0.3);
        params->setAttributeValue("LOWER_BOUND", lb);

        NOMAD::ArrayOfDouble ub(n, 0.3);
        ...
        std::string history_name = "DPAM1_" + std::to_string(ctype) + "_dmultimads.txt";
        ...
        auto ev = std::make_unique<DPAM1>(params->getEvalParams(), ctype);
