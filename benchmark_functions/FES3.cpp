...
class FES3 : public NOMAD::Evaluator
{
  public:
    explicit FES3(const std::shared_ptr<NOMAD::EvalParameters> &evalParams, const size_t n, const size_t ctype)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB), _n(n), _ctype(ctype)
    {}

    ~FES3() override = default;

    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        bool eval_ok = false;

        NOMAD::Double f1 = 1e20, f2 = 1e20, f3 = 1e20, f4 = 1e20;
        ...
        try
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                f1 += pow(abs(xi[i] - exp(((i + 1.0) / n) * ((i + 1.0) / n)) / 3), 0.5);
            }

            f2 = 0;
            for (int i = 0; i < n; ++i)
            {
                f2 += pow(abs(xi[i] - sin(i) * sin(i) * cos(i) * cos(i)), 0.5);
            }

            f3 = 0;
            for (int i = 0; i < n; ++i)
            {
                f3 += pow(abs(xi[i] - 0.25 * cos((long double)i) * cos(2.0 * i) - 0.5), 0.5);
            }

            f4 = 0;
            for (int i = 0; i < n; ++i)
            {
                const double tmp = x[i].todouble() - 0.5 * sin(1000 * PI * (i + 1.0) / n) - 0.5;
                f4 += tmp * tmp;
            }

            std::string bbo = f1.tostring();
            bbo += " " + f2.tostring();
            bbo += " " + f3.tostring();
            bbo += " " + f4.tostring();
            bbo += " ";
...
bool run_pb(const size_t n, const size_t ctype)
{
    ...
    try
    {
        ...
        auto ev = std::make_unique<FES3>(params->getEvalParams(), n, ctype);
