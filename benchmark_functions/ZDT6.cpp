...
class ZDT6 : public NOMAD::Evaluator
{
  public:
    explicit ZDT6(const std::shared_ptr<NOMAD::EvalParameters> &evalParams, const size_t n, const size_t ctype)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB), _n(n), _ctype(ctype)
    {
    }

    ~ZDT6() override = default;
    ...
    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        ...
        try
        {
            ...
            // Objectives
            f1 = 1 - exp(-4 * xi[0]) * pow(sin(6 * PI * xi[0]), 6);
            ...
            tmp_g /= (n - 1);
            const double g = 1.0 + 9 * pow(tmp_g, 0.25);
...
bool run_pb(const size_t n, const size_t ctype)
{
    ...
    try
    {
        ...
        auto ev = std::make_unique<ZDT6>(params->getEvalParams(), n, ctype);
