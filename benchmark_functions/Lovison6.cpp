...
class Lovison6 : public NOMAD::Evaluator
{
  public:
    explicit Lovison6(const std::shared_ptr<NOMAD::EvalParameters> &evalParams, const size_t ctype)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB), _n(3), _ctype(ctype)
    {
    }

    ~Lovison6() override = default;
    ...
    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        ...
        {
            const size_t n = 3;
            const size_t m = 4; // Number of functions used

            // Parameters
            const double PI = 3.141592653589793238463;

            // Random matrix
            const double C[3][4] = {{0.218418, -0.620254, 0.843784, 0.914311},
                                    {-0.788548, 0.428212, 0.103064, -0.47373},
                                    {-0.300792, -0.185507, 0.330423, 0.151614}};

            const double alpha[4][3] = {{0.942022, 0.363525, 0.00308876},
                                        {0.755598, 0.450103, 0.170122},
                                        {0.787748, 0.837808, 0.590166},
                                        {0.203093, 0.253639, 0.532339}};

            // Random parameters
            const double beta[] = {-0.666503, -0.945716, -0.334582, 0.611894};
            const double gamma[] = {0.281032, 0.508749, -0.0265389, -0.920133};
            ...
            // Objectives
            f1 = f_tmp[0] + beta[0] * exp(f_tmp[3] / gamma[0]);
...
bool run_pb(const size_t ctype)
{
    ...
    try
    {
        ...
        std::string history_name = "lovison6_" + std::to_string(ctype) + "_dmultimads.txt";
        ...
        auto ev = std::make_unique<Lovison6>(params->getEvalParams(), ctype);
