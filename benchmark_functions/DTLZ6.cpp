...
class DTLZ6 : public NOMAD::Evaluator
{
  public:
    explicit DTLZ6(const std::shared_ptr<NOMAD::EvalParameters> &evalParams, const size_t n, const size_t m,
                   const size_t ctype)
    ...
    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        ...
        try
        {
            ...
            for (int i = 0; i < M - 1; ++i)
            {
                f[M - 1] -= (xi[i] / (1 + g)) * (1 + sin(3 * PI * xi[i]));
            }
            f[M - 1] *= (1 + g);
...
bool run_pb(const size_t n, const size_t m, const size_t ctype)
{
    ...
    try
    {
        ...
        auto ev = std::make_unique<DTLZ6>(params->getEvalParams(), n, m, ctype);
