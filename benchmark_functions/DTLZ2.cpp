...
class DTLZ2 : public NOMAD::Evaluator
{
    public:
        explicit DTLZ2(const std::shared_ptr<NOMAD::EvalParameters>& evalParams,
                const size_t n,
                const size_t m,
                const size_t ctype)
...
bool run_pb(const size_t n, const size_t m,const size_t ctype)
{
    ...
    try
    {
        ...
        auto ev = std::make_unique<DTLZ2>(params->getEvalParams(), n, m, ctype);
