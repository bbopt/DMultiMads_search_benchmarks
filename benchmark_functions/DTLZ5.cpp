...
class DTLZ5 : public NOMAD::Evaluator
{
    public:
        explicit DTLZ5(const std::shared_ptr<NOMAD::EvalParameters>& evalParams,
                const size_t n,
                const size_t m,
                const size_t ctype)
        ...
        bool eval_x(NOMAD::EvalPoint& x, const NOMAD::Double& hMax, bool& countEval) const override
        {
            ...
            try
            {
                ...
                else if (_ctype == 3)
                {
                    ...
                    for (int j = 0; j < l; ++j)
                    {
                        NOMAD::Double c = xi[j]*xi[j] + xi[j+1]*xi[j+1] + xi[j]*xi[j+1] - 2*xi[j] - 2*xi[j+1] + 1;
                        bbo += c.tostring() + " ";
                    }
                }
                else if (_ctype == 4)
...
bool run_pb(const size_t n, const size_t m, const size_t ctype)
{
    ...
    try
    {
        ...
        auto ev = std::make_unique<DTLZ5>(params->getEvalParams(), n, m, ctype);
