...
class ZDT1 : public NOMAD::Evaluator
{
  public:
    explicit ZDT1(const std::shared_ptr<NOMAD::EvalParameters> &evalParams, const size_t n, const size_t ctype)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB), _n(n), _ctype(ctype)
    {
    }

    ~ZDT1() override = default;
...
bool run_pb(const size_t n, const size_t ctype)
{
    ...
    try
    {
        ...
        auto ev = std::make_unique<ZDT1>(params->getEvalParams(), n, ctype);
