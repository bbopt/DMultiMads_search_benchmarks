...
class TKLY1 : public NOMAD::Evaluator
{
  public:
    explicit TKLY1(const std::shared_ptr<NOMAD::EvalParameters> &evalParams, const size_t n, const size_t ctype)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB), _n(n), _ctype(ctype)
    {
    }

    ~TKLY1() override = default;
    ...
    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        ...
        try
        {
            ...
            for (int i = 1; i < n; ++i)
            {
                f2 *= (2.0 - exp(-((xi[i] - 0.1) / 0.004) * (xi[i] - 0.1) / 0.004) -
                       0.8 * exp(-((xi[i] - 0.9) / 0.4) * (xi[i] - 0.9) / 0.4));
            }
            f2 /= xi[0];
...
bool run_pb(const size_t n, const size_t ctype)
{
    ...
    try
    {
        ...
        std::string history_name = "TKLY1_";
        ...
        auto ev = std::make_unique<TKLY1>(params->getEvalParams(), n, ctype);
...
{
    const size_t n = 4;
    std::vector<size_t> ctype{0, 1, 3, 4, 5, 6};
