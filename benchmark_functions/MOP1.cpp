...
class MOP1 : public NOMAD::Evaluator
{
  public:
    explicit MOP1(const std::shared_ptr<NOMAD::EvalParameters> &evalParams)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB)
    {
    }

    ~MOP1() override = default;
...
bool run_pb()
{
    ...
    try
    {
        ...
        NOMAD::ArrayOfDouble lb(n, -1e5);
        params->setAttributeValue("LOWER_BOUND", lb);

        NOMAD::ArrayOfDouble ub(n, 1e5);
        ...
        std::string history_name = "MOP1_dmultimads.txt";
        ...
        auto ev = std::make_unique<MOP1>(params->getEvalParams());
