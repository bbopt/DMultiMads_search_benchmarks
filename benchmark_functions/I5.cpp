...
class I5 : public NOMAD::Evaluator
{
  public:
    explicit I5(const std::shared_ptr<NOMAD::EvalParameters> &evalParams, const size_t n, const size_t m,
                const size_t ctype)
    ...
    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        ...
        try
        {
            ...
            // r_sum
            std::vector<double> r_sum(n, 0);
            r_sum[0] = 0;
            for (int i = 1; i < n; ++i)
            {
                long double r_sum_num = 0.0;
                long double r_sum_denom = 0.0;
                for (int j = 0; j < i; ++j)
                {
                    r_sum_num += w[j] * y[j];
                    r_sum_denom += w[j];
                }
                r_sum[i] = r_sum_num / r_sum_denom;
            }
...
bool run_pb(const size_t n, const size_t m, const size_t ctype)
{
    ...
    try
    {
        ...
        auto ev = std::make_unique<I5>(params->getEvalParams(), n, m, ctype);
