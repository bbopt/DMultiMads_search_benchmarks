...
class MOP5 : public NOMAD::Evaluator
{
  public:
    explicit MOP5(const std::shared_ptr<NOMAD::EvalParameters> &evalParams)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB)
    {
    }

    ~MOP5() override = default;
    ...
    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        ...
        try
        {
            ...
            // Objectives
            f1 = 0.5 * (x1 * x1 + x2 * x2) + sin(x1 * x1 + x2 * x2);
            f2 = (3 * x1 - 2 * x2 + 4) * (3 * x1 - 2 * x2 + 4) / 8 + (x1 - x2 + 1) * (x1 - x2 + 1) / 27 + 15;
            f3 = 1 / (x1 * x1 + x2 * x2 + 1) - 1.1 * exp(-x1 * x1 - x2 * x2);
...
bool run_pb()
{
    ...
    try
    {
        ...
        NOMAD::ArrayOfDouble lb(n, -30);
        params->setAttributeValue("LOWER_BOUND", lb);

        NOMAD::ArrayOfDouble ub(n, 30);
        ...
        std::string history_name = "MOP5_dmultimads.txt";
        ...
        auto ev = std::make_unique<MOP5>(params->getEvalParams());
