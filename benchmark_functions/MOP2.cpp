...
class MOP2 : public NOMAD::Evaluator
{
  public:
    explicit MOP2(const std::shared_ptr<NOMAD::EvalParameters> &evalParams, const size_t n, const size_t ctype)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB), _n(n), _ctype(ctype)

    {
    }

    ~MOP2() override = default;
    ...
    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        ...
        try
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                tmp_f1 += -(xi[i] - 1.0 / sqrt(n)) * (xi[i] - 1.0 / sqrt(n));
            }
            f1 = 1 - exp(tmp_f1);
            ...
            for (int i = 0; i < n; ++i)
            {
                tmp_f2 += -(xi[i] + 1.0 / sqrt(n)) * (xi[i] + 1.0 / sqrt(n));
            }
            f2 = 1 - exp(tmp_f2);
...
bool run_pb(size_t n, size_t ctype)
{
    ...
    try
    {
        ...
        std::string history_name = "MOP2_" + std::to_string(ctype) + "_dmultimads.txt";
        ...
        auto ev = std::make_unique<MOP2>(params->getEvalParams(), n, ctype);
