...
class Kursawe : public NOMAD::Evaluator
{
  public:
    explicit Kursawe(const std::shared_ptr<NOMAD::EvalParameters> &evalParams, const size_t n, const size_t ctype)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB), _n(n), _ctype(ctype)
    {
    }

    ~Kursawe() override = default;
    ...
    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        ...
        try
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                f2 += pow(abs(xi[i]), 0.8) + 5 * pow(sin(xi[i]), 3);
            }
...
bool run_pb(const size_t n, const size_t ctype)
{
    ...
    try
    {
        ...
        std::string history_name = "Kursawe_";
        ...
        auto ev = std::make_unique<Kursawe>(params->getEvalParams(), n, ctype);
