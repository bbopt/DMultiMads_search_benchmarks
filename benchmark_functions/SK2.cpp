...
class SK2 : public NOMAD::Evaluator
{
  public:
    explicit SK2(const std::shared_ptr<NOMAD::EvalParameters> &evalParams, const size_t ctype)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB), _n(4), _ctype(ctype)

    {
    }

    ~SK2() override = default;
    ...
    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        ...
        try
        {
            ...
            // Objectives
            f1 = -(xi[0] - 2) * (xi[0] - 2) - (xi[1] + 3) * (xi[1] + 3) - (xi[2] - 5) * (xi[2] - 5) -
                 (xi[3] - 4) * (xi[3] - 4) + 5;
            ...
            for (int i = 0; i < _n; ++i)
            {
                tmp_num += sin(x[i].todouble());
                tmp_denom += x[i].todouble() * x[i].todouble() / 100;
            }
            f2 = tmp_num / tmp_denom;
...
bool run_pb(size_t ctype)
{
    ...
    try
    {
        ...
        NOMAD::ArrayOfDouble lb(n, -10);
        params->setAttributeValue("LOWER_BOUND", lb);

        NOMAD::ArrayOfDouble ub(n, 10);
        ...
        std::string history_name = "SK2_" + std::to_string(ctype) + "_dmultimads.txt";
        ...
        auto ev = std::make_unique<SK2>(params->getEvalParams(), ctype);
