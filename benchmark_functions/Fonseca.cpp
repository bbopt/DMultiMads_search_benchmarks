...
class Fonseca : public NOMAD::Evaluator
{
  public:
    explicit Fonseca(const std::shared_ptr<NOMAD::EvalParameters> &evalParams)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB)
    {
    }

    ~Fonseca() override = default;
    ...
    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        ...
        try
        {
            ...
            // Objectives
            f1 = 1 - exp(-(x1 - 1) * (x1 - 1) - (x2 + 1) * (x2 + 1));
            f2 = 1 - exp(-(x1 + 1) * (x1 + 1) - (x2 - 1) * (x2 - 1));
...
bool run_pb()
{
    ...
    try
    {
        ...
        NOMAD::ArrayOfDouble lb(n, -4.0);
        params->setAttributeValue("LOWER_BOUND", lb);

        NOMAD::ArrayOfDouble ub(n, 4.0);
        ...
        std::string history_name = "Fonseca_dmultimads.txt";
        ...
        auto ev = std::make_unique<Fonseca>(params->getEvalParams());
