...
class WFG9 : public NOMAD::Evaluator
{
  public:
    explicit WFG9(const std::shared_ptr<NOMAD::EvalParameters> &evalParams, const size_t n, const size_t m,
                  const size_t ctype)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB), _n(n), _m(m), _ctype(ctype)
    {
    }

    ~WFG9() override = default;
    ...
    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        ...
        try
        {
            ...
            const double AA = 0.98 / 49.98;
            const double BB = 0.02;
            const double CC = 50;

            std::vector<double> r_sum(n - 1, 0);
            for (int i = 0; i < n - 1; ++i)
            {
                long double tmp_num = 0;
                long double tmp_denom = 0;
                for (int j = i + 1; j < n; ++j)
                {
                    tmp_num += w[j] * y[j];
                    tmp_denom += w[j];
                }
                r_sum[i] = tmp_num / tmp_denom;
            }

            std::vector<double> t1(n, 0);
            for (int i = 0; i < n - 1; ++i)
            {
                t1[i] = pow(y[i], BB + (CC - BB) * (AA - (1 - 2 * r_sum[i]) * abs(floor(0.5 - r_sum[i]) + AA)));
            }
            t1[n - 1] = y[n - 1];

            // Second level mapping
            const double AAA = 0.35;
            const double BBB = 0.001;
            const double CCC = 0.05;
            const double AAAA = 30;
            const double BBBB = 95;
            const double CCCC = 0.35;

            std::vector<double> t2(n, 0);
            for (int i = 0; i < k; ++i)
            {
                t2[i] = 1 + (abs(t1[i] - AAA) - BBB) *
                                ((floor(t1[i] - AAA + BBB) * (1 - CCC + (AAA - BBB) / BBB)) / (AAA - BBB) +
                                 (floor(AAA + BBB - t1[i]) * (1 - CCC + (1 - AAA - BBB) / BBB)) / (1 - AAA - BBB) +
                                 1 / BBB);
            }
            for (int i = k; i < n; ++i)
            {
                t2[i] = (1 + cos((4 * AAAA + 2) * PI * (0.5 - abs(t1[i] - CCCC) / (2 * (floor(CCCC - t1[i]) + CCCC)))) +
                         4 * BBBB * pow(abs(t1[i] - CCCC) / (2 * (floor(CCCC - t1[i]) + CCCC)), 2)) /
                        (BBBB + 2);
            }

            // Third level mapping
            std::vector<double> t3(M, 0);
            for (int i = 0; i < M - 1; ++i)
            {
                t3[i] = 0;
                int tmp_i = i + 1;
                for (int ii = ((tmp_i - 1) * k / (M - 1) + 1) - 1; ii <= (tmp_i * k / (M - 1)) - 1; ++ii)
                {
                    t3[i] += t2[ii];
                    int tmp_ii = ii + 1;
                    for (int jj = -1; jj <= (k / (M - 1) - 2) - 1; jj++)
                    {
                        int tmp_jj = jj + 1;
                        t3[i] += abs(t2[ii] - t2[((tmp_i - 1) * k / (M - 1) + 1) +
                                                 ((tmp_ii + tmp_jj - ((tmp_i - 1) * k / (M - 1) + 1) + 1) %
                                                  ((tmp_i * k / (M - 1)) - ((tmp_i - 1) * k / (M - 1) + 1) + 1)) -
                                                 1]);
                    }
                }
                t3[i] /= ((tmp_i * k / (M - 1)) - ((tmp_i - 1) * k / (M - 1) + 1) + 1) / (k / (M - 1)) *
                         ceil(k / (M - 1) / 2) * (1 + 2 * k / (M - 1) - 2 * ceil(k / (M - 1) / 2));
            }
            t3[M - 1] = 0;
            for (int ii = k; ii < n; ++ii)
            {
                t3[M - 1] += t2[ii];
                int tmp_ii = ii + 1;
                for (int jj = -1; jj <= (l - 2) - 1; jj++)
                {
                    int tmp_jj = jj + 1;
                    t3[M - 1] += abs(t2[ii] - t2[k + 1 + ((tmp_ii + tmp_jj - (k + 1) + 1) % (n - k)) - 1]);
                }
            }
            t3[M - 1] /= ((n - k) / l) * ceil(l / 2) * (1 + 2 * l - 2 * ceil(l / 2));

            // Define objective functions variables
            std::vector<double> x_tmp(M, 0);
            for (int i = 0; i < M - 1; ++i)
            {
                x_tmp[i] = fmax(t3[M - 1], A[i]) * (t3[i] - 0.5) + 0.5;
            }
            x_tmp[M - 1] = t3[M - 1];
            ...
            for (int m = 1; m < M - 1; m++)
            {
                ...
                {
                    h[m] *= sin(x_tmp[i] * PI / 2);
                }
                h[m] *= cos(x_tmp[M - m - 1] * PI / 2);
            }
            h[M - 1] = cos(x_tmp[0] * PI / 2);
...
bool run_pb(const size_t n, const size_t m, const size_t ctype)
{
    ...
    try
    {
        ...
        auto ev = std::make_unique<WFG9>(params->getEvalParams(), n, m, ctype);
