...
class I3 : public NOMAD::Evaluator
{
  public:
    explicit I3(const std::shared_ptr<NOMAD::EvalParameters> &evalParams, const size_t n, const size_t m,
                const size_t ctype)
    ...
    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        ...
        try
        {
            ...
            for (int i = 1; i < n; ++i)
            {
                double r_sum_num = 0.0;
                double r_sum_denom = 0.0;
                for (int j = 0; j < i; ++j)
                {
                    r_sum_num += w[j] * y[j];
                    r_sum_denom += w[j];
            ...
            for (int i = 0; i < M; ++i)
            {
                long double tmp_t3_num = 0.0;
                long double tmp_t3_denom = 0.0;
                if (i < M - 1)
                {
                    int tmp_i = i + 1;
...
bool run_pb(const size_t n, const size_t m, const size_t ctype)
{
    ...
    try
    {
        ...
        auto ev = std::make_unique<I3>(params->getEvalParams(), n, m, ctype);
