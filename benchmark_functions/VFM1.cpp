...
class VFM1 : public NOMAD::Evaluator
{
  public:
    explicit VFM1(const std::shared_ptr<NOMAD::EvalParameters> &evalParams)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB)
    {
    }

    ~VFM1() override = default;
    ...
    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        ...
        try
        {
            ...
            // Objectives
            f1 = x1 * x1 + (x2 - 1) * (x2 - 1);
            f2 = x1 * x1 + (x2 + 1) * (x2 + 1) + 1;
            f3 = (x1 - 1) * (x1 - 1) + x2 * x2 + 2;
...
bool run_pb()
{
    ...
    try
    {
        ...
        std::string history_name = "VFM1_dmultimads.txt";
        ...
        auto ev = std::make_unique<VFM1>(params->getEvalParams());
