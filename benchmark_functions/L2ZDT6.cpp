...
class L2ZDT6 : public NOMAD::Evaluator
{
  public:
    explicit L2ZDT6(const std::shared_ptr<NOMAD::EvalParameters> &evalParams, const size_t ctype)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB), _n(10), _ctype(ctype)

    {
    }

    ~L2ZDT6() override = default;
...
bool run_pb(size_t ctype)
{
    ...
    try
    {
        ...
        std::string history_name = "L2ZDT6_" + std::to_string(ctype) + "_dmultimads.txt";
        ...
        auto ev = std::make_unique<L2ZDT6>(params->getEvalParams(), ctype);
