...
class Deb53 : public NOMAD::Evaluator
{
    public:
        explicit Deb53(const std::shared_ptr<NOMAD::EvalParameters>& evalParams)
            : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB)
        {}

        ~Deb53() override = default;
        ...
        bool eval_x(NOMAD::EvalPoint& x, const NOMAD::Double& hMax, bool& countEval) const override
        {
            ...
            try
            {
                ...
                // Objectives
                f1 = 1 - exp(-4 * x1) * pow(sin(5 * PI * x1), 4);
                double g = 0.0;
                if (x2 <= 0.4)
                {
...
bool run_pb()
{
    ...
    try
    {
        ...
        std::string history_name = "Deb53_dmultimads.txt";
        ...
        auto ev = std::make_unique<Deb53>(params->getEvalParams());
