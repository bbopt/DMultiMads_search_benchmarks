...
class Deb513 : public NOMAD::Evaluator
{
    public:
        explicit Deb513(const std::shared_ptr<NOMAD::EvalParameters>& evalParams)
            : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB)
        {}

        ~Deb513() override = default;
        ...
        bool eval_x(NOMAD::EvalPoint& x, const NOMAD::Double& hMax, bool& countEval) const override
        {
            ...
            try
            {
                ...
                const double g = 1 + 10 * x2;
                const double h = 1 - pow(f1.todouble() / g, alpha) - (f1.todouble() / g) * sin(2 * pi * q * f1.todouble());
                f2 = g * h;
...
bool run_pb()
{
    ...
    try
    {
        ...
        params->setAttributeValue("DMULTIMADS_NM_SEARCH_USE_DOM", false);
        params->setAttributeValue("DMULTIMADS_QUAD_MODEL_SEARCH", true);
        params->setAttributeValue("DMULTIMADS_QUAD_MODEL_SEARCH_USE_DOM", true);
        params->setAttributeValue("DMULTIMADS_QUAD_DMS_SEARCH", false);
        ...
        std::string history_name = "Deb513_dmultimads.txt";
        ...
        auto ev = std::make_unique<Deb513>(params->getEvalParams());
