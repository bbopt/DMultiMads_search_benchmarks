...
class Far1 : public NOMAD::Evaluator
{
    public:
        explicit Far1(const std::shared_ptr<NOMAD::EvalParameters>& evalParams)
            : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB)
        {}

        ~Far1() override = default;
        ...
        bool eval_x(NOMAD::EvalPoint& x, const NOMAD::Double& hMax, bool& countEval) const override
        {
            ...
            try
            {
                ...
                // Objectives
                f1 = -2 * exp(15 * (-(x1 - 0.1) * (x1 - 0.1) - x2 * x2))
                    - exp(20 * (-(x1 - 0.6) * (x1 - 0.6) - (x2 - 0.6) * (x2 - 0.6)))
                    + exp(20 * (-(x1 + 0.6) * (x1 + 0.6) - (x2 - 0.6) * (x2 - 0.6)))
                    + exp(20 * (-(x1 - 0.6) * (x1 - 0.6) - (x2 + 0.6) * (x2 + 0.6)))
                    + exp(20 * (-(x1 + 0.6) * (x1 + 0.6) - (x2 + 0.6) * (x2 + 0.6)));
                f2 = 2 * exp(20 * (-x1 * x1 - x2 * x2))
                    + exp(20 * (-(x1 - 0.4) * (x1 - 0.4) - (x2 - 0.6) * (x2 - 0.6)))
                    - exp(20 * (-(x1 + 0.5) * (x1 + 0.5) - (x2 - 0.7) * (x2 - 0.7)))
                    - exp(20 * (-(x1 - 0.5) * (x1 - 0.5) - (x2 + 0.7) * (x2 + 0.7)))
                    + exp(20 * (-(x1 + 0.4) * (x1 + 0.4) - (x2 + 0.8) * (x2 + 0.8)));
...
bool run_pb()
{
    ...
    try
    {
        ...
        std::string history_name = "Far1_dmultimads.txt";
        ...
        auto ev = std::make_unique<Far1>(params->getEvalParams());
