...
class Lovison4 : public NOMAD::Evaluator
{
  public:
    explicit Lovison4(const std::shared_ptr<NOMAD::EvalParameters> &evalParams)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB)
    {
    }

    ~Lovison4() override = default;
    ...
    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        ...
        try
        {
            ...
            // Objectives
            f1 = -x1 * x1 - x2 * x2 - 4 * (exp(-(x1 + 2) * (x1 + 2) - x2 * x2) + exp(-(x1 - 2) * (x1 - 2) - x2 * x2));
            f2 = -(x1 - 6.0) * (x1 - 6.0) - (x2 + 0.5) * (x2 + 0.5);
...
bool run_pb()
{
    ...
    try
    {
        ...
        std::string history_name = "lovison4_dmultimads.txt";
        ...
        auto ev = std::make_unique<Lovison4>(params->getEvalParams());
