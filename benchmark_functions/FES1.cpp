...
class FES1 : public NOMAD::Evaluator
{
  public:
    explicit FES1(const std::shared_ptr<NOMAD::EvalParameters> &evalParams, const size_t n, const size_t ctype)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB), _n(n), _ctype(ctype)
    {
    }

    ~FES1() override = default;
    ...
    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        ...
        try
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                long double tmp = xi[i] - 0.5 * cos(10 * PI * (i + 1.0) / n) - 0.5;
                f2 += tmp * tmp;
            }
...
bool run_pb(const size_t n, const size_t ctype)
{
    ...
    try
    {
        ...
        auto ev = std::make_unique<FES1>(params->getEvalParams(), n, ctype);
