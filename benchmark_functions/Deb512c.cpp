...
class Deb512c : public NOMAD::Evaluator
{
    public:
        explicit Deb512c(const std::shared_ptr<NOMAD::EvalParameters>& evalParams)
            : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB)
        {}

        ~Deb512c() override = default;
        ...
        bool eval_x(NOMAD::EvalPoint& x, const NOMAD::Double& hMax, bool& countEval) const override
        {
            ...
            try
            {
                ...
                if (x2 <= 0.4)
                {
                    g = 4 - 3 * exp(-((x2 - 0.2) / 0.02) * ((x2 - 0.2) / 0.02));
                }
                else
                {
                    g = 4 - 2 * exp(-((x2 - 0.7) / 0.2) * ((x2 - 0.7) / 0.2));
                }

                const double alpha = 0.25 + 3.75 * (g-1);
                double h = 0;
                if (f1 <= beta * g)
                {
                    h = 1 - std::pow(f1.todouble() / (beta * g), alpha);
                }
                f2 = g * h;
...
bool run_pb()
{
    ...
    try
    {
        ...
        std::string history_name = "Deb512c_dmultimads.txt";
        ...
        auto ev = std::make_unique<Deb512c>(params->getEvalParams());
