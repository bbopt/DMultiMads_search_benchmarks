...
class Jin4 : public NOMAD::Evaluator
{
  public:
    explicit Jin4(const std::shared_ptr<NOMAD::EvalParameters> &evalParams, const size_t n, const size_t ctype)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB), _n(n), _ctype(ctype)
    {
    }

    ~Jin4() override = default;
    ...
    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        ...
        try
        {
            ...
            for (int i = 1; i < n; ++i)
            {
                g += (9.0 / (n - 1)) * x[i].todouble();
            }
            f2 = g * (1.0 - pow(f1.todouble() / g, 0.25) - pow(f1.todouble() / g, 4));
...
bool run_pb(const size_t n, const size_t ctype)
{
    ...
    try
    {
        ...
        auto ev = std::make_unique<Jin4>(params->getEvalParams(), n, ctype);
