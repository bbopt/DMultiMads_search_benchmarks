...
class Deb41 : public NOMAD::Evaluator
{
    public:
        explicit Deb41(const std::shared_ptr<NOMAD::EvalParameters>& evalParams)
            : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB)
        {}

        ~Deb41() override = default;
        ...
        bool eval_x(NOMAD::EvalPoint& x, const NOMAD::Double& hMax, bool& countEval) const override
        {
            ...
            try
            {
                ...
                // Objectives
                double gx = 2 - (exp(-((x2 - 0.2) / 0.004) * ((x2 - 0.2) / 0.004)));
                gx -= 0.8 * exp(-((x2 - 0.6) / 0.4) * ((x2 - 0.6) / 0.4));
                f1 = x1;
                f2 = gx / f1;
...
bool run_pb()
{
    ...
    try
    {
        ...
        std::string history_name = "Deb41_dmultimads.txt";
        ...
        auto ev = std::make_unique<Deb41>(params->getEvalParams());
