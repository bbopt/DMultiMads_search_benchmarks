...
class MOP4 : public NOMAD::Evaluator
{
  public:
    explicit MOP4(const std::shared_ptr<NOMAD::EvalParameters> &evalParams, const size_t n, const size_t ctype)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB), _n(n), _ctype(ctype)

    {
    }

    ~MOP4() override = default;
    ...
    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        ...
        try
        {
            ...
            for (int i = 0; i < n - 1; ++i)
            {
                f1 += -10 * exp(-0.2 * sqrt(xi[i] * xi[i] + xi[i + 1] * xi[i + 1]));
            }
            f2 = 0;
            for (int i = 0; i < n; ++i)
            {
                f2 += pow(abs(xi[i]), 0.8) + 5 * sin(pow(xi[i], 3));
            }
...
bool run_pb(size_t n, size_t ctype)
{
    ...
    try
    {
        ...
        std::string history_name = "MOP4_" + std::to_string(ctype) + "_dmultimads.txt";
        ...
        auto ev = std::make_unique<MOP4>(params->getEvalParams(), n, ctype);
