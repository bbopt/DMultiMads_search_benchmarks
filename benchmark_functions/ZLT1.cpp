...
class ZLT1 : public NOMAD::Evaluator
{
  public:
    explicit ZLT1(const std::shared_ptr<NOMAD::EvalParameters> &evalParams, const size_t n, const size_t m,
                  const size_t ctype)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB), _n(n), _m(m), _ctype(ctype)
    {
    }

    ~ZLT1() override = default;
...
bool run_pb(const size_t n, const size_t m, const size_t ctype)
{
    ...
    try
    {
        ...
        NOMAD::ArrayOfDouble lb(n, -1000);
        params->setAttributeValue("LOWER_BOUND", lb);

        NOMAD::ArrayOfDouble ub(n, 1000);
        ...
        auto ev = std::make_unique<ZLT1>(params->getEvalParams(), n, m, ctype);
