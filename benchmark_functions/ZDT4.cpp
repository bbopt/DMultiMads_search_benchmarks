...
class ZDT4 : public NOMAD::Evaluator
{
  public:
    explicit ZDT4(const std::shared_ptr<NOMAD::EvalParameters> &evalParams, const size_t n, const size_t ctype)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB), _n(n), _ctype(ctype)
    {
    }

    ~ZDT4() override = default;
...
bool run_pb(const size_t n, const size_t ctype)
{
    ...
    try
    {
        ...
        auto ev = std::make_unique<ZDT4>(params->getEvalParams(), n, ctype);
