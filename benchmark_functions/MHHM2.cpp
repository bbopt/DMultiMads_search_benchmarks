...
class MHHM2 : public NOMAD::Evaluator
{
  public:
    explicit MHHM2(const std::shared_ptr<NOMAD::EvalParameters> &evalParams)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB)
    {
    }

    ~MHHM2() override = default;
    ...
    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        ...
        try
        {
            ...
            // Objectives
            f1 = (x1 - 0.8) * (x1 - 0.8) + (x2 - 0.6) * (x2 - 0.6);
            f2 = (x1 - 0.85) * (x1 - 0.85) + (x2 - 0.7) * (x2 - 0.7);
            f3 = (x1 - 0.9) * (x1 - 0.9) + (x2 - 0.6) * (x2 - 0.6);
...
bool run_pb()
{
    ...
    try
    {
        ...
        std::string history_name = "MHHM2_dmultimads.txt";
        ...
        auto ev = std::make_unique<MHHM2>(params->getEvalParams());
