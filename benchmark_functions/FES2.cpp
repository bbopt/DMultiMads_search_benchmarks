...
class FES2 : public NOMAD::Evaluator
{
  public:
    explicit FES2(const std::shared_ptr<NOMAD::EvalParameters> &evalParams, const size_t n, const size_t ctype)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB), _n(n), _ctype(ctype)

    {
    }

    ~FES2() override = default;
    ...
    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        ...
        try
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                const double tmp = xi[i] - 0.5 * cos(10 * PI * (i + 1.0) / n) - 0.5;
                f1 += tmp * tmp;
            }
            ...
            for (int i = 0; i < n; ++i)
            {
                f3 += pow(abs(xi[i] - 0.25 * cos(i) * cos(2.0 * i) - 0.5), 0.5);
            }
...
bool run_pb(const size_t n, const size_t ctype)
{
    ...
    try
    {
        ...
        auto ev = std::make_unique<FES2>(params->getEvalParams(), n, ctype);
