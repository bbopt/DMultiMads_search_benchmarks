...
class WFG2 : public NOMAD::Evaluator
{
  public:
    explicit WFG2(const std::shared_ptr<NOMAD::EvalParameters> &evalParams, const size_t n, const size_t m,
                  const size_t ctype)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB), _n(n), _m(m), _ctype(ctype)
    {
    }

    ~WFG2() override = default;
    ...
    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        ...
        try
        {
            ...
            for (int i = k; i < k + l / 2; ++i)
            {
                t2[i] = 0;
                int tmp_i = i + 1;
                for (int ii = k + 2 * (tmp_i - k) - 1 - 1; ii <= k + 2 * (tmp_i - k) - 1; ++ii)
                {
                    t2[i] += t1[ii];
                    int tmp_ii = ii + 1;
                    for (int jj = -1; jj <= AA - 2 - 1; ++jj)
                    {
                        int tmp_jj = jj + 1;
                        t2[i] += abs(t1[ii] - t1[(k + 2 * (tmp_i - k) - 1) +
                                                 ((tmp_ii + tmp_jj - (k + 2 * (tmp_i - k) - 1) + 1) %
                                                  (k + 2 * (tmp_i - k) - (k + 2 * (tmp_i - k) - 1) + 1)) -
                                                 1]);
                    }
                }
                t2[i] /= (k + 2 * (tmp_i - k) - (k + 2 * (tmp_i - k) - 1) + 1) / AA * ceil(AA / 2) *
                         (1 + 2 * AA - 2 * ceil(AA / 2));
            }
            ...
            h[M - 1] =
                1 - pow(x_tmp[0], alpha) * cos(AAA * PI * pow(x_tmp[0], beta)) * cos(AAA * PI * pow(x_tmp[0], beta));
...
bool run_pb(const size_t n, const size_t m, const size_t ctype)
{
    ...
    try
    {
        ...
        auto ev = std::make_unique<WFG2>(params->getEvalParams(), n, m, ctype);
