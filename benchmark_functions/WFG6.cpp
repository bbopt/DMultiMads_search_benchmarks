...
class WFG6 : public NOMAD::Evaluator
{
  public:
    explicit WFG6(const std::shared_ptr<NOMAD::EvalParameters> &evalParams, const size_t n, const size_t m,
                  const size_t ctype)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB), _n(n), _m(m), _ctype(ctype)
    {
    }

    ~WFG6() override = default;
        ...
        try
        {
            ...
            for (int i = 0; i < M - 1; ++i)
            {
                ...
                for (int ii = ((tmp_i - 1) * k / (M - 1) + 1) - 1; ii <= (tmp_i * k / (M - 1)) - 1; ++ii)
                {
                    ...
                    {
                        int tmp_jj = jj + 1;
                        t2[i] += abs(t1[ii] - t1[((tmp_i - 1) * k / (M - 1) + 1) +
                                                 ((tmp_ii + tmp_jj - ((tmp_i - 1) * k / (M - 1) + 1) + 1) %
                                                  ((tmp_i * k / (M - 1)) - ((tmp_i - 1) * k / (M - 1) + 1) + 1)) -
                                                 1]);
                    }
                }
                t2[i] /= ((tmp_i * k / (M - 1)) - ((tmp_i - 1) * k / (M - 1) + 1) + 1) / (k / (M - 1)) *
            ...
            for (int ii = k; ii < n; ++ii)
            {
                ...
                {
                    int tmp_jj = jj + 1;
                    t2[M - 1] += abs(t1[ii] - t1[k + 1 + ((tmp_ii + tmp_jj - (k + 1) + 1) % (n - k)) - 1]);
                }
            }
            t2[M - 1] /= ((n - k) / l) * ceil(l / 2) * (1 + 2 * l - 2 * ceil(l / 2));

            // Define objective functions variables
            std::vector<double> x_tmp(M, 0);
            for (int i = 0; i < M - 1; ++i)
            {
                x_tmp[i] = fmax(t2[M - 1], A[i]) * (t2[i] - 0.5) + 0.5;
            }
            x_tmp[M - 1] = t2[M - 1];
...
bool run_pb(const size_t n, const size_t m, const size_t ctype)
{
    ...
    try
    {
        ...
        auto ev = std::make_unique<WFG6>(params->getEvalParams(), n, m, ctype);
