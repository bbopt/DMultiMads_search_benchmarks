...
class WFG3 : public NOMAD::Evaluator
{
  public:
    explicit WFG3(const std::shared_ptr<NOMAD::EvalParameters> &evalParams, const size_t n, const size_t m,
                  const size_t ctype)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB), _n(n), _m(m), _ctype(ctype)
    {
    }

    ~WFG3() override = default;
...
bool run_pb(const size_t n, const size_t m, const size_t ctype)
{
    ...
    try
    {
        ...
        auto ev = std::make_unique<WFG3>(params->getEvalParams(), n, m, ctype);
