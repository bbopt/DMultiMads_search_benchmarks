...
class SK1 : public NOMAD::Evaluator
{
  public:
    explicit SK1(const std::shared_ptr<NOMAD::EvalParameters> &evalParams)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB)
    {
    }

    ~SK1() override = default;
    ...
    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        ...
        try
        {
            ...
            // Objectives
            f1 = -x1 * x1 * x1 * x1 - 3 * x1 * x1 * x1 + 10 * x1 * x1 + 10 * x1 + 10;
            f2 = 0.5 * x1 * x1 * x1 * x1 + 2 * x1 * x1 * x1 + 10 * x1 * x1 - 10 * x1 + 5;
...
bool run_pb()
{
    ...
    try
    {
        ...
        std::string history_name = "SK1_dmultimads.txt";
        ...
        auto ev = std::make_unique<SK1>(params->getEvalParams());
