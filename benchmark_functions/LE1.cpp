...
class LE1 : public NOMAD::Evaluator
{
  public:
    explicit LE1(const std::shared_ptr<NOMAD::EvalParameters> &evalParams)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB)
    {
    }

    ~LE1() override = default;
    ...
    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        ...
        try
        {
            ...
            // Objectives
            f1 = pow(x1 * x1 + x2 * x2, 0.125);
            f2 = pow((x1 - 0.5) * (x1 - 0.5) + (x2 - 0.5) * (x2 - 0.5), 0.25);
...
bool run_pb()
{
    ...
    try
    {
        ...
        std::string history_name = "LE1_dmultimads.txt";
        ...
        auto ev = std::make_unique<LE1>(params->getEvalParams());
