...
class MLF2 : public NOMAD::Evaluator
{
  public:
    explicit MLF2(const std::shared_ptr<NOMAD::EvalParameters> &evalParams)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB)
    {
    }

    ~MLF2() override = default;
    ...
    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        ...
        try
        {
            ...
            // Objectives
            f1 = 5 - ((x1 * x1 + x2 - 11) * (x1 * x1 + x2 - 11) + (x1 + x2 * x2 - 7) * (x1 + x2 * x2 - 7)) / 200;
            f2 = 5 - ((4 * x1 * x1 + 2 * x2 - 11) * (4 * x1 * x1 + 2 * x2 - 11) +
                      (2 * x1 + 4 * x2 * x2 - 7) * (2 * x1 + 4 * x2 * x2 - 7)) /
                         200;
...
bool run_pb()
{
    ...
    try
    {
        ...
        NOMAD::ArrayOfDouble lb(n, -2);
        params->setAttributeValue("LOWER_BOUND", lb);

        NOMAD::ArrayOfDouble ub(n, 2);
        ...
        std::string history_name = "MLF2_dmultimads.txt";
        ...
        auto ev = std::make_unique<MLF2>(params->getEvalParams());
