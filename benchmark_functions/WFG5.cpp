...
class WFG5 : public NOMAD::Evaluator
{
  public:
    explicit WFG5(const std::shared_ptr<NOMAD::EvalParameters> &evalParams, const size_t n, const size_t m,
                  const size_t ctype)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB), _n(n), _m(m), _ctype(ctype)
    {
    }

    ~WFG5() override = default;
    ...
    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        ...
        try
        {
            ...
            // First level mapping
            const double AA = 0.35;
            const double BB = 0.001;
            const double CC = 0.05;
            std::vector<double> t1(n, 0);
            for (int i = 0; i < n; ++i)
            {
                t1[i] = 1 + (abs(y[i] - AA) - BB) *
                                ((floor(y[i] - AA + BB) * (1 - CC + (AA - BB) / BB)) / (AA - BB) +
                                 (floor(AA + BB - y[i]) * (1 - CC + (1 - AA - BB) / BB)) / (1 - AA - BB) + 1 / BB);
            }
            ...
            for (int i = 0; i < M; ++i)
            {
                long double tmp_t2_num = 0.0;
                long double tmp_t2_denom = 0.0;
                if (i < M - 1)
                {
                    int tmp_i = i + 1;
...
bool run_pb(const size_t n, const size_t m, const size_t ctype)
{
    ...
    try
    {
        ...
        auto ev = std::make_unique<WFG5>(params->getEvalParams(), n, m, ctype);
