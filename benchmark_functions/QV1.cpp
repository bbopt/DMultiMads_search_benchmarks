...
class QV1 : public NOMAD::Evaluator
{
  public:
    explicit QV1(const std::shared_ptr<NOMAD::EvalParameters> &evalParams, const size_t n, const size_t ctype)
        : NOMAD::Evaluator(evalParams, NOMAD::EvalType::BB), _n(n), _ctype(ctype)
    {
    }

    ~QV1() override = default;
    ...
    bool eval_x(NOMAD::EvalPoint &x, const NOMAD::Double &hMax, bool &countEval) const override
    {
        ...
        try
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                tmp_f1 += (xi[i] * xi[i] - 10.0 * cos(2 * PI * xi[i]) + 10) / n;
            }
            f1 = pow(tmp_f1, 0.25);
            ...
            for (int i = 0; i < n; ++i)
            {
                tmp_f2 += ((xi[i] - 1.5) * (xi[i] - 1.5) - 10.0 * cos(2 * PI * (xi[i] - 1.5)) + 10) / n;
            }
            f2 = pow(tmp_f2, 0.25);
...
bool run_pb(const size_t n, const size_t ctype)
{
    ...
    try
    {
        ...
        NOMAD::ArrayOfDouble lb(n, -5.12);
        params->setAttributeValue("LOWER_BOUND", lb);

        NOMAD::ArrayOfDouble ub(n, 5.12);
        ...
        auto ev = std::make_unique<QV1>(params->getEvalParams(), n, ctype);
